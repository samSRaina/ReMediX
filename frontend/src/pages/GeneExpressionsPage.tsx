      ...
      <Surface>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {payload.data.map((row: Record<string, string | number | null>, idx: number) => (
                  <tr key={`${row['Gene.symbol'] || 'g'}-${idx}`} className="border-t border-slate-100">
        ...
        {payload && payload.total_pages > 1 ? (
          <div className="mt-4 flex items-center justify-center gap-2">
            <button className="rounded-lg border border-slate-300 px-2 py-1 text-sm" disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>Prev</button>
            <span className="text-sm text-slate-600">Page {payload.page} / {payload.total_pages}</span>
            <button className="rounded-lg border border-slate-300 px-2 py-1 text-sm" disabled={page >= payload.total_pages} onClick={() => setPage((p) => Math.min(payload.total_pages, p + 1))}>Next</button>
          </div>
        ) : null}
      </Surface>
    </AppLayout>
